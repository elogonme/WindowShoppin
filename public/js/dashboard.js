$(document).ready(() => {
  const dateToday = moment().format('ll'); // get todays date using moment.js
  // Make call to API on server to get currencies and output to page
  $.get('/api/currencies').then((data) => {
  ...
  $('.check-update').on('click', async function () {
    ...
    // Start spinner
    $('.loader').removeClass('invisible');
    await checkItemforUpdate(itemId, () => {});
    window.location.replace('/dashboard');
  });
